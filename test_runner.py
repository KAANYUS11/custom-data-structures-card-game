...
NESTED_TESTCASE_INPUT_DIR = os.path.join("testcases", "testcase_inputs")
NESTED_TESTCASE_OUTPUT_DIR = os.path.join("testcases", "testcase_outputs")
MAIN_CLASS = "Main"
...
def get_test_files(test_type=None):
    ...
    input_dir = TESTCASE_INPUT_DIR
    if not glob.glob(os.path.join(input_dir, "*.txt")) and os.path.exists(NESTED_TESTCASE_INPUT_DIR):
        input_dir = NESTED_TESTCASE_INPUT_DIR
...
def run_single_test(input_file, verbose=False, benchmark=False):
    ...
    expected_dir = TESTCASE_OUTPUT_DIR
    if not os.path.exists(os.path.join(expected_dir, f"{basename}.txt")) and os.path.exists(NESTED_TESTCASE_OUTPUT_DIR):
        expected_dir = NESTED_TESTCASE_OUTPUT_DIR

    expected_file = os.path.join(expected_dir, f"{basename}.txt")
